...
If Steam has several Valheim installations, the script asks which one to use. A path can also be specified explicitly:

    .\tools\BuildAndInstall.ps1 -GamePath "C:\Program Files (x86)\Steam\steamapps\common\Valheim"

Use `-NoInstall` to only build and create the ZIP. The game must be closed while the new DLL is copied.
...
The mod also posts each sent check and received item to the Archipelago room chat. Sent checks and received items are additionally shown in the local Valheim chat under the Archipelago name. When a check sends an item to another slot, the local chat also shows the item name and recipient. Messages written by other Archipelago players and server chat messages are mirrored into the local Valheim chat as well. This makes the actual runtime check and reward names visible to everyone without relying on a manually maintained list in the mod UI.
